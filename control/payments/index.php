...
if (isset($_SESSION['admin_login']) && $_SESSION['admin_login'] === true) {
  ...
  global $pdo;
  $stmt = $pdo->prepare('SELECT * FROM payment_methods LIMIT ?, ?');
  $stmt->execute([
    $offset,
    ROW_PER_PAGE
  ]);
  ...
  if ($stmt->rowCount()) {
        ...
        <div class="nk-tb-col">
          <a href="">
            <div class="user-card">
              ...
              <div class="user-info">
                <span class="tb-lead"><?= $payment['name'] ?> <span class="dot dot-success d-md-none ml-1"></span></span>
              </div>
            </div>
          </a>
        </div>
            ...
            <li>
              <div class="drodown">
                ...
                <div class="dropdown-menu dropdown-menu-right">
                  <ul class="link-list-opt no-bdr">
                    <li>
                      <a href="#" class="edit-payment" data-edit-payment="<?= $payment['id']?>"><em class="icon ni ni-edit"></em><span>Edit</span></a>
                    </li>
                    <li>
                      <a href="#" class="delete-payment" data-delete-payment="<?= $payment['id']?>"><em class="icon ni ni-trash"></em><span>Delete</span></a>
                    </li>
                  </ul>
                </div>
              </div>
    ...
    <?php
    $stmt = $pdo->prepare('SELECT id FROM payment_methods');
    $stmt->execute();
