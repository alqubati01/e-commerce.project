...
else
{
                ...
                <div class="nk-block">
                  ...
                  <div class="card">
                    <div class="card-inner">
                      <form id="edit-product" class="form-validate">
                        <div class="row g-gs">
                          <div class="col-md-6">
                            <div class="col-md-12 m-0 p-0">
                              <input type="hidden" id="fv-id">
                          ...
                          <div class="col-md-12">
                            ...
                            <div class="mb-2 d-flex" id="product-images">
                          ...
                          <div class="col-md-12">
                            <div class="form-group">
                              <button type="submit" id="save-product" class="btn btn-primary">Save</button>
                            </div>
                          </div>
                        </div>
                      </form>
  ...
  <script>
    ...
    $(document).ready(function(){
      ...
      comboTree3 = $('.categories_list').comboTree({
      ...
      function brandChange(id) {
        comboTree3.onChange(function() {
          $.ajax({
            ...
            method: "GET",
            data: {id: comboTree3.getSelectedIds()},
            setData: {brandId: id},
            success: function (data) {
              $("#fv-brand").html(data);
              $("#fv-brand").val(this.setData.brandId).change();
            }
          });
        });
      }
      ...
      function loadData() {
        $.ajax({
          async: false,
          url: "./control/products/get_product.php",
          method: "GET",
          data: {id: params.get('id')},
          dataType: 'json',
          success: function (data) {
            console.log(data);
            $("#fv-id").val(data.basic[0].id);
            $("#fv-name").val(data.basic[0].productName);
            $("#fv-qty").val(data.basic[0].qty);
            $("#fv-sku").val(data.basic[0].sku);
            $("#fv-price").val(data.basic[0].price);
            $("#fv-cost").val(data.basic[0].product_cost);
            brandChange(data.basic[0].brandId);
            comboTree3.setSelection(data.categories.map(m => m.category_id));
            quill.setContents(JSON.parse(data.basic[0].description));

            // data.attachments.forEach((file) => {
            //   myDropzone.options.addedfile.call(myDropzone, file);
            //   myDropzone.options.thumbnail.call(myDropzone, file, "http://127.0.0.1/Ecommerce/assets/" + file.path);
            // });

            var html = "";
            data.attachments.forEach((file) => {
              path = './assets/' + file.path;
              deletePath = './control/products/delete_image.php?id=' + file.id;
              html += "<div class='d-flex flex-column mr-2'>";
              html += "<img src=" + path + " alt='' class='rounded mb-1' />";
              html += "<a href=" + deletePath + " class='btn btn-danger'>Delete</a>";
              html += "</div>";
            });
      ...
      let myDropzone = new Dropzone('.dropzone', {
        url: "./control/products/update_product.php",
        ...
        init: function() {
          ...
          document.getElementById("save-product").addEventListener("click", function(e) {
            ...
            if (dzClosure.getQueuedFiles().length === 0) {
              var blob = new Blob();
              blob.upload = { 'chunked': dzClosure.chunking };
          ...
          this.on("sending", function(file, xhr, formData){
            ...
            formData.append("productId", jQuery("#fv-id").val());
            ...
            formData.append("categoryIds", comboTree3.getSelectedIds());
